...
export async function generateWeeklyPlan(): Promise<void> {
  ...
  if (!settings.photos_folder_id && !settings.videos_folder_id) {
    console.log("[Agent] Folders not configured — skipping plan generation");
    return;
  }
  ...
  if (existing && existing.length > 0) {
    console.log(`[Agent] Plan already exists for week ${weekStart}`);
    return;
  }
...
Available photos (${photos.length} files):
${photos.slice(0, 50).map((p) => `${p.id}: ${p.name} (${p.createdTime})`).join("\n")}

Available videos (${videos.length} files):
${videos.slice(0, 30).map((v) => `${v.id}: ${v.name} (${v.createdTime})`).join("\n")}
...
Respond ONLY with valid JSON:
{
  "suggestions": [
    ...
    console.log(`[Agent] Week ${weekStart} plan generated — ${rows.length} suggestions`);
  } catch (err) {
    console.error("[Agent] generateWeeklyPlan error:", err);
    if (runId) {
      await adminSupabase
        .from("content_agent_runs")
